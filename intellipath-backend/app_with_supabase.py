...
def create_syllabus():
    ...
    # Initialisation de l'agent d'enseignement
    teaching_agent = TeachingAgent()
    teaching_agent.seed_agent(syllabus_content, topic)
...
def conversation(session_id):
    ...
    # Fournir l'historique des messages à l'agent
    for msg in messages:
        if msg["role"] == "user":
            teaching_agent.respond(msg["content"])
    ...
    db.track_progress(
        ...
        status='in_progress',
        completion_percentage=min(100, len(messages) * 5)  # Progression basée sur le nombre de messages
    )
...
def submit_quiz(quiz_id):
    ...
    if module_id:
        module_progress = db.track_progress(
            user_id=user_id,
            syllabus_id=syllabus_id,
            module_id=module_id,
            status='in_progress',
            completion_percentage=min(100, (score / len(quiz_data['questions'])) * 100 if quiz_data['questions'] else 0)
        )
    ...
    module_ids = set([m['id'] for m in db.get_modules(syllabus_id)])
    completed_modules = 0
    ...
    syllabus_completion = (completed_modules / len(module_ids)) * 100 if module_ids else 0
    
    # Mise à jour de la progression du syllabus
    syllabus_progress = db.track_progress(
        user_id=user_id,
        syllabus_id=syllabus_id,
        module_id=None,
